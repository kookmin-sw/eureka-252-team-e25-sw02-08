...
Why? (왜?) → 현실 세계의 모든 사물과 정보를 디지털 정보와 자연스럽게 연결해서, 사람이 굳이 컴퓨터 앞에 앉지 않아도 언제 어디서든 "정보와 상호작용"할 수 있도록 하기 위함.
...
기존 (As-Is) : 키보드 → 손으로 직접 타이핑, 마우스 → 클릭/드래그, 모니터 → 고정된 디스플레이
Sixth Sense (To-Be) : 손 제스처, 공중 타이핑 (손가락 추적), 손가락 움직임으로 화면 제어 (예: 집기 동작, 스크롤), 어디든 투사되는 화면 (벽, 종이, 손바닥 등)

-현재
현재 투사되어 단순히 손가락이나 몸의 움직임을 이용해서 사물을 추적할거나 사진을 찍는 듯한 행위는 불가능하지만, 눈의 움직임을 이용해 문자를 입력하는 기술, 스마트워치의 경우에는 손을 씻는 듯한 행위를 하면 씻는 것으로 인식하는 등의 기술은 현재 구현이 된 상태이다. 따라서 MIT Sixth Sense 프로젝트의 아이디어는 계속해 계승, 발전되었다고 할 수 있다.
