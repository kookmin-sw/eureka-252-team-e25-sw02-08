...
- ai→ 해킹 당하나요? ai 인프라를 노리는 공격을 받은 기업 30퍼 프롬프트유출이나 학습한 것
...
* AI 어업 시스템에 대한 관점 → 아이디어는 참신하지만 **비용 문제**가 핵심 쟁점
  ...
  * 교수님 질문: 어느 쪽이 더 쌀까?
  * 팀 의견: **인건비가 더 저렴할 것이라 예상**

* 에너지 비용 이슈 → 에너지 부족, 정전 경험, 국가 차원의 전력 사용 절감 요구

  * AI 도입 시 **전력·에너지 비용 문제**가 실제 적용의 장애 요소
  * 결론: AI는 **인건비보다 저렴**하고 **에너지 문제가 해결**되어야 실사용 가능

* 차세대 컴퓨팅 디바이스 → **홀로그래픽스**

  * 스마트폰은 기술 성숙으로 체감 발전이 적음
  * 휴대폰을 대체할 **차세대 디바이스로 홀로그래픽스** 제시
  * LLM + 카메라 기술을 접목해 발전 중

* 홀로포테이션 기술 현황

  * 현재: HMD(머리 착용 디스플레이) + 다수의 카메라 필요
  * 미래: 안경 수준의 디바이스로 대체 가능성
  * AI 추론을 통해 **2D → 3D 변환**, 카메라 1대로 구현 가능성

* 콘택트 렌즈 디스플레이에 대한 견해 → **현실적으로 어려움**

  * 인간 시각 인식 메커니즘 문제

    * 수정체 두께 조절 + 눈 각도 조절이 후천적으로 동기화됨
  * 실제 디스플레이는 가까운데 뇌는 멀다고 인식 → **인지 부조화**
  * 결과: 어지러움, 구역질 발생 가능

* 해결 가능성 및 한계

  * 뉴럴 네트워크로 보정 가능성은 존재
  * 하지만 **생체 해킹 위험**을 감수할 만큼 필요한 기술인지 의문 제기
  * 팀 결론: 해당 기술은 **실질적 발전 가능성 낮음**

* 인터뷰를 통해 얻은 인사이트

  * 기술 발전 수준에 대한 전문가 시각 이해
  * 기술 자체보다 **비용과 필요성**이 기술 도입의 핵심 요소임을 인식
  * 단순하지만 중요한 관점에서 AI 기술을 다시 생각하게 됨
